...
/**
 * <p>
 * This interface is used to define cases that can be used to match a given long value and define associated actions that are
 * executed when a case matches for the given value. The actions and therefore the whole matching will provide no result
 * value.<br>
 * The long value to be checked has to be provided during creation of the instance. Only the action of one of the given cases
 ...
 * <ul>
 * <li> {@link de.boereck.matcher.NoResultLongCaseMatcher#otherwise(java.util.function.LongConsumer) otherwise(Consumer)}</li>
 * <li> {@link de.boereck.matcher.NoResultLongCaseMatcher#otherwiseThrow(java.util.function.Supplier) otherwiseThrow(Supplier)}</li>
 * </ul>
 ...
 * @author Max Bureck
 */
public interface NoResultLongCaseMatcher {
     ...
     * 
     * @param reference
     *            value that is checked to be equal to the input value. If they are equal the case matches
     * @param consumer
     *            action that will be executed (with input object as parameter) when the case is determined to be the
     *            matching one.
